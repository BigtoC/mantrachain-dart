//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:mantrachain_dart_sdk/api.dart';
import 'package:test/test.dart';

// tests for TokenfactoryParams200ResponseParams
void main() {
  // final instance = TokenfactoryParams200ResponseParams();

  group('test TokenfactoryParams200ResponseParams', () {
    // DenomCreationFee defines the fee to be charged on the creation of a new denom. The fee is drawn from the MsgCreateDenom's sender account, and transferred to the community pool.
    // List<TokenfactoryParams200ResponseParamsDenomCreationFeeInner> denomCreationFee (default value: const [])
    test('to test the property `denomCreationFee`', () async {
      // TODO
    });

    // DenomCreationGasConsume defines the gas cost for creating a new denom. This is intended as a spam deterrence mechanism.  See: https://github.com/CosmWasm/token-factory/issues/11
    // String denomCreationGasConsume
    test('to test the property `denomCreationGasConsume`', () async {
      // TODO
    });

    // String feeCollectorAddress
    test('to test the property `feeCollectorAddress`', () async {
      // TODO
    });


  });

}
