//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:openapi/api.dart';
import 'package:test/test.dart';

// tests for CosmosBankV1beta1QuerySpendableBalancesResponse
void main() {
  // final instance = CosmosBankV1beta1QuerySpendableBalancesResponse();

  group('test CosmosBankV1beta1QuerySpendableBalancesResponse', () {
    // balances is the spendable balances of all the coins.
    // List<Coin> balances (default value: const [])
    test('to test the property `balances`', () async {
      // TODO
    });

    // QueryCodesResponseIsTheResponseTypeForTheQueryCodesRPCMethodPagination pagination
    test('to test the property `pagination`', () async {
      // TODO
    });


  });

}
