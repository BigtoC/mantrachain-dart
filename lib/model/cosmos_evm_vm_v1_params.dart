...
class CosmosEvmVmV1Params {
  /// Returns a new [CosmosEvmVmV1Params] instance.
  CosmosEvmVmV1Params({
    this.evmDenom,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is CosmosEvmVmV1Params &&
  ...
  @override
  String toString() => 'CosmosEvmVmV1Params[evmDenom=$evmDenom, extraEips=$extraEips, chainConfig=$chainConfig, allowUnprotectedTxs=$allowUnprotectedTxs, evmChannels=$evmChannels, accessControl=$accessControl, activeStaticPrecompiles=$activeStaticPrecompiles]';
  ...
  /// Returns a new [CosmosEvmVmV1Params] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static CosmosEvmVmV1Params? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        for (var key in requiredKeys) {
          assert(json.containsKey(key), 'Required key "CosmosEvmVmV1Params[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "CosmosEvmVmV1Params[$key]" has a null value in JSON.');
        }
        return true;
      }());

      return CosmosEvmVmV1Params(
  ...
  static List<CosmosEvmVmV1Params> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <CosmosEvmVmV1Params>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = CosmosEvmVmV1Params.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, CosmosEvmVmV1Params> mapFromJson(dynamic json) {
    final map = <String, CosmosEvmVmV1Params>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = CosmosEvmVmV1Params.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of CosmosEvmVmV1Params-objects as value to a dart map
  static Map<String, List<CosmosEvmVmV1Params>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<CosmosEvmVmV1Params>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = CosmosEvmVmV1Params.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
