...
class SinceCosmosSdk047Header {
  /// Returns a new [SinceCosmosSdk047Header] instance.
  SinceCosmosSdk047Header({
    this.version,
    this.chainId,
    this.height,
    this.time,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is SinceCosmosSdk047Header &&
  ...
  /// Returns a new [SinceCosmosSdk047Header] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static SinceCosmosSdk047Header? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "SinceCosmosSdk047Header[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "SinceCosmosSdk047Header[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return SinceCosmosSdk047Header(
  ...
  static List<SinceCosmosSdk047Header> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <SinceCosmosSdk047Header>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = SinceCosmosSdk047Header.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, SinceCosmosSdk047Header> mapFromJson(dynamic json) {
    final map = <String, SinceCosmosSdk047Header>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = SinceCosmosSdk047Header.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of SinceCosmosSdk047Header-objects as value to a dart map
  static Map<String, List<SinceCosmosSdk047Header>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<SinceCosmosSdk047Header>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = SinceCosmosSdk047Header.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
